  ...
  <form on:submit|preventDefault={handleSubmit} autocomplete="off" class="space-y-4">
    <fieldset disabled={isSubmitting} class="space-y-4">
      <div>
        ...
        <input
          id="{idPrefix}_name" type="text" bind:value={form.name} required placeholder="Masukkan nama sertifikat"
      ...
      <div>
        <label for="{idPrefix}_no_certificate" class="block text-sm font-medium text-slate-900 dark:text-slate-100">No Sertifikat</label>
        <input
          id="{idPrefix}_no_certificate" type="text" bind:value={form.no_certificate} required placeholder="Masukkan nomor sertifikat"
          class="mt-1 block w-full rounded-md border border-black/10 dark:border-white/10
                 bg-white/80 dark:bg-[#0e0c19]/80 px-3 py-2 text-sm text-slate-900 dark:text-slate-100
                 focus:outline-none focus:ring-2 focus:ring-violet-500"
        />
      ...
      {#if showProjectSelect}
        <div>
          <label for="{idPrefix}_project" class="block text-sm font-medium text-slate-900 dark:text-slate-100">Project</label>
          <select
            id="{idPrefix}_project" bind:value={form.project_id} required
          ...
          >
            ...
            {#each projects as p (p.id)}<option value={p.id}>{p.name ?? p.title}</option>{/each}
          </select>
        </div>
      {/if}

      <div>
        <label for="{idPrefix}_barang_certificate" class="block text-sm font-medium text-slate-900 dark:text-slate-100">Barang Certificate</label>
        <select
          id="{idPrefix}_barang_certificate" bind:value={form.barang_certificate_id} required
          class="mt-1 block w-full rounded-md border border-black/10 dark:border-white/10
                 bg-white/80 dark:bg-[#0e0c19]/80 px-3 py-2 text-sm text-slate-900 dark:text-slate-100
                 focus:outline-none focus:ring-2 focus:ring-violet-500"
        ...
        >
          ...
          {#each barangOptions as b (b.id)}<option value={b.id}>{b.name ?? b.title}{#if b.no_seri} - {b.no_seri}{/if}</option>{/each}
        </select>
        {#if (showProjectSelect ? form.project_id : true) && barangOptions.length === 0}
          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Tidak ada Barang Certificate untuk Project ini</p>
        {/if}
      </div>
      ...
      <div>
        ...
        <select
          id="{idPrefix}_status" bind:value={form.status} required
          class="mt-1 block w-full rounded-md border border-black/10 dark:border-white/10
                 bg-white/80 dark:bg-[#0e0c19]/80 px-3 py-2 text-sm text-slate-900 dark:text-slate-100
                 focus:outline-none focus:ring-2 focus:ring-violet-500"
        >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label for="{idPrefix}_issue" class="block text-sm font-medium text-slate-900 dark:text-slate-100">Tanggal Terbit</label>
          <input
            id="{idPrefix}_issue" type="date" bind:value={form.date_of_issue}
            class="mt-1 block w-full rounded-md border border-black/10 dark:border-white/10
                   bg-white/80 dark:bg-[#0e0c19]/80 px-3 py-2 text-sm text-slate-900 dark:text-slate-100
                   focus:outline-none focus:ring-2 focus:ring-violet-500"
          />
        ...
        <div>
          <label for="{idPrefix}_expired" class="block text-sm font-medium text-slate-900 dark:text-slate-100">Tanggal Expired</label>
          <input
            id="{idPrefix}_expired" type="date" bind:value={form.date_of_expired}
            class="mt-1 block w-full rounded-md border border-black/10 dark:border-white/10
                   bg-white/80 dark:bg-[#0e0c19]/80 px-3 py-2 text-sm text-slate-900 dark:text-slate-100
                   focus:outline-none focus:ring-2 focus:ring-violet-500"
          />
    ...
    <div class="pt-2">
      <button
        type="submit"
        class="w-full flex items-center justify-center gap-2 rounded-md bg-violet-600 px-3 py-2 text-sm font-semibold text-white
