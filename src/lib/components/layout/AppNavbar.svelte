<script lang="ts">
    ...
    const links: Link[] = [
      ...
      { name: 'Barang Certificates', href: '/barang-certificates' },
      { name: 'Certificates',        href: '/certificates' }
    ];
  
    // Visible / overflow logic for "More"
    ...
    function recalc() {
      // Mobile break handled purely by CSS; this only runs for md+ layouts
      ...
      // Prepare: render everything visible to measure
      visible = [...links];
      overflow = [];
      const containerWidth = navEl.clientWidth;
      const rightWidth = rightEl.clientWidth;
      const logoWidth = logoEl.clientWidth;
      const moreWidth = 88; // rough width reservation for "More" button
      const spacing = 24;   // paddings/gaps buffer
  
      const available = containerWidth - rightWidth - logoWidth - spacing;
  
      // Measure cumulative width of link buttons (approx using getBoundingClientRect)
      // Move items into overflow until muat
      let used = 0;
      ...
      for (const l of links) {
        ...
        // Leave space for "More" if something will overflow
      ...
      // If there is overflow, ensure "More" itself fits. If not, move last visible to overflow
      if (tmpOverflow.length) {
        while (used + moreWidth > available && tmpVisible.length) {
          const moved = tmpVisible.pop()!;
          tmpOverflow.unshift(moved); // keep order
          // approximate width reduce: re-measure roughly by char count
    ...
    onMount(() => {
      ...
      const listener = () => { showMore = false; showProfile = false; };
  ...
  <div bind:this={navEl} class="sticky top-0 z-50" role="banner">
        ...
        <div class="flex h-[64px] items-center justify-between gap-2">
          ...
          <div class="flex items-center gap-3 min-w-0" bind:this={leftEl}>
            ...
            <div class="hidden md:flex items-center gap-1">
              {#each visible as l (l.href)}
                <button
                  ...
                  class="px-3 py-2 rounded-xl text-sm font-medium transition
                         ...
                         {isActive(l.href, $page.url.pathname) ? 'text-violet-700 dark:text-violet-300 bg-violet-600/10' : 'text-slate-600 dark:text-slate-300'}">
                  {l.name}
                </button>
              {/each}
              ...
              {#if overflow.length}
                <div class="relative">
                  <button bind:this={moreBtnEl}
                          on:click|stopPropagation={() => (showMore = !showMore)}
                          class="px-3 py-2 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-violet-700 dark:hover:text-violet-300 hover:bg-violet-600/10">
                    More
                  </button>
                  {#if showMore}
          ...
          <div class="flex items-center gap-2" bind:this={rightEl}>
            ...
            <!-- Profile dropdown -->
            <div class="relative">
              <button on:click|stopPropagation={() => (showProfile = !showProfile)}
            ...
            <button class="md:hidden inline-flex items-center justify-center h-10 w-10 rounded-xl border border-black/5 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5"
    ...
    {#if showMobile}
      <div class="fixed inset-0 z-50" aria-modal="true" role="dialog">
        ...
        <div class="absolute inset-0 bg-black/50" on:click={() => (showMobile = false)}></div>
        <div class="absolute right-0 top-0 h-full w-[88%] max-w-sm bg-white/90 dark:bg-[#0e0c19]/90 backdrop-blur border-l border-black/5 dark:border-white/10 shadow-2xl">
          <div class="flex items-center justify-between px-4 py-4">
            <span class="font-semibold tracking-wide text-violet-700 dark:text-violet-300">INDOGREEN</span>
            <!-- svelte-ignore a11y_consider_explicit_label -->
            <button class="h-10 w-10 rounded-xl border border-black/5 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5"
          ...
          <div class="px-2 space-y-1">
            {#each links as l (l.href)}
              <button on:click={() => go(l.href)}
                class="w-full text-left px-3 py-3 rounded-xl text-sm font-medium
                       text-slate-700 dark:text-slate-200 hover:bg-violet-600/10 hover:text-violet-700 dark:hover:text-violet-300">
                {l.name}
              </button>
            {/each}
          </div>
  
          <div class="px-2 pt-3 border-t border-black/5 dark:border-white/10 mt-3">
            <button on:click={() => go('/settings')}
                    class="w-full text-left px-3 py-3 rounded-xl text-sm text-slate-700 dark:text-slate-200 hover:bg-violet-600/10 hover:text-violet-700 dark:hover:text-violet-300">
              Pengaturan
            </button>
            <button on:click={logout}
                    class="w-full text-left px-3 py-3 rounded-xl text-sm text-red-600 dark:text-red-400 hover:bg-red-500/10">
              Logout
            </button>
          </div>
        </div>
